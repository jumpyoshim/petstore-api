...
package: guard-S3BUCKET
	...
	@sam package --s3-bucket ${S3BUCKET} --template-file template.yaml --output-template-file packaged.yaml
deploy: package
	@echo "start deploy"
	@sam deploy --template-file packaged.yaml  --stack-name ${STACK_NAME} --capabilities CAPABILITY_IAM --parameter-overrides \
